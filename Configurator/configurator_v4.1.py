...
# Minimum clearance between *adjacent prop tips* (same rotor plane), mm
PROP_PROP_CLEARANCE_MM = 20.0
...
def compute():
    ...
    # Nominal motor radius (for tube clearance aesthetics)
    motor_radius_nominal = prop_radius + 0.3 * prop_mm  # tweak 0.3 as needed

    # Minimum motor radius to avoid prop overlap, based on polygon spacing
    # Distance between adjacent motors: D = 2 * R * sin(pi / n)
    # Need D >= prop_mm + PROP_PROP_CLEARANCE_MM
    theta_half = math.pi / float(n_motors)
    min_motor_radius_spacing = (prop_mm + PROP_PROP_CLEARANCE_MM) / (
        2.0 * math.sin(theta_half)
    )

    # Final motor radius: satisfy both clearance-to-tube needs and prop spacing
    motor_radius = max(motor_radius_nominal, min_motor_radius_spacing)

    # Prop tip radius from center (with possibly updated motor_radius)
    ...
    status_lines = []
    # Geometry spacing status
    if motor_radius > motor_radius_nominal + 1e-6:
        status_lines.append(
            f"Geometry: Motor radius increased from {motor_radius_nominal:.0f} mm "
            f"to {motor_radius:.0f} mm to avoid prop overlap "
            f"(min spacing rule with {PROP_PROP_CLEARANCE_MM:.0f} mm prop–prop clearance)."
        )
    else:
        status_lines.append(
            "Geometry: Motor radius satisfies prop spacing with chosen clearance factor."
        )
    ...
    oshape_lines = [
        ...
        f"#motor_radius_nominal = {motor_radius_nominal:.0f} mm",
        f"#motor_radius = {motor_radius:.0f} mm  // center to motor (safe spacing)",
        f"#prop_radius = {prop_radius:.0f} mm",
        f"#prop_tip_radius = {prop_tip_radius:.0f} mm",
        f"#clearance_prop_to_tube = {clearance_mm:.0f} mm",
        f"#prop_prop_clearance = {PROP_PROP_CLEARANCE_MM:.0f} mm",
    ...
    lines.append("")
    lines.append(f"Motor radius nominal (center → motor): {motor_radius_nominal:.0f} mm")
    lines.append(f"Motor radius safe (used): {motor_radius:.0f} mm")
    lines.append(f"Prop radius: {prop_radius:.0f} mm")
    lines.append(f"Prop-tip radius: {prop_tip_radius:.0f} mm")
    lines.append(f"Prop–prop clearance requirement: {PROP_PROP_CLEARANCE_MM:.0f} mm")
...
# ---------------------------------------------------------------------
root = tk.Tk()
root.title("Drone Prop / Frame Configurator v4.1 (safe spacing)")
...
# Row 7: Canvas label
ttk.Label(main_frame, text="Top view layout (tube + props, safe spacing):").grid(
